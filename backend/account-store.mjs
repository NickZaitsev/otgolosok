...
export function createAccountStore(db, now = Date.now) {
  ...
  return {
    ...
    listWalks(userId, limit=20) { const rows=db.prepare("SELECT * FROM user_walks WHERE user_id=? ORDER BY updated_at DESC,id DESC LIMIT ?").all(userId,Math.min(50,Math.max(1,limit+1)));return { walks:rows.slice(0,limit).map(viewWalk),hasMore:rows.length>limit }; },
    ...
    listFavorites(userId,limit=50) { return db.prepare("SELECT object_type AS type,object_id AS id,created_at AS createdAt FROM user_favorites WHERE user_id=? ORDER BY created_at DESC LIMIT ?").all(userId,Math.min(50,limit)); },
    ...
    listRequests(userId,limit=50) { return db.prepare("SELECT job_id AS jobId,operation,created_at AS createdAt FROM user_generation_requests WHERE user_id=? ORDER BY created_at DESC,id DESC LIMIT ?").all(userId,Math.min(50,limit)); },

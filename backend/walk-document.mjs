...
export function migrateLegacyDraft(value, walkId) {
  if (!object(value) || value.version !== 1 || !uuid(walkId) || !(value.start === null || place(value.start)) || !Array.isArray(value.stops) || value.stops.length > 5 || !value.stops.every(place) || (value.start === null && value.stops.length !== 0) || !["open", "loop"].includes(value.mode) || ![30, 60, 90].includes(value.minutes) || !Array.isArray(value.jobs) || value.jobs.length > 100) throw invalid();
  const hash = (index) => {

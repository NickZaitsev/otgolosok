...
test("review rejection gets one rewrite and then stops",async t=>{const f=fixture(t);f.queue[3]={value:{approved:false,issues:["Неподтверждённая дата"],checks:{substantive:false,subjectAligned:true,audioClear:true},paragraphFacts:[],claims:[]}};f.queue.push({text:paragraph},{value:{approved:false,issues:["Дата осталась"],checks:{substantive:false,subjectAligned:true,audioClear:true},paragraphFacts:[],claims:[]}});let voiced=false;f.options.narrate=async()=>{voiced=true;};const job=await runJob(f.store.claimNext(),f.options);assert.equal(job.stage,"review_required");assert.equal(publicJob(job).story,null);assert.equal(voiced,false);});

test("short supported response becomes description without TTS",async t=>{const f=fixture(t,{factCount:1});const short="Памятник создан в Москве и посвящён важному событию городской истории. Архивный источник подтверждает автора, время создания и первоначальное место установки этой небольшой мемориальной композиции для жителей города и посетителей.";f.queue[2]={text:short};f.queue[3]={value:{approved:true,issues:[],checks:{substantive:true,subjectAligned:true,audioClear:true},paragraphFacts:[{paragraph:1,factIds:["f1"]}],claims:[{paragraph:1,text:"Памятник создан в Москве",factIds:["f1"],supported:true,address:false}]}};let voiced=false;f.options.narrate=async()=>{voiced=true;};const job=await runJob(f.store.claimNext(),f.options);assert.equal(job.stage,"ready");assert.equal(job.data.story.effectiveProfile,"description-v1");assert.equal(job.data.story.downgradeReason,"insufficient_material_for_story");assert.equal(voiced,false);});
...
test("unfinished legacy evidence and draft are revalidated while fetched sources are reused",async t=>{
  const f=fixture(t),initial=f.store.claimNext();
  initial.data={sources:[{id:"s1",url:f.urls[0],title:"Источник",publisher:"one.example",text:f.page}],evidence:{placeName:"Старый дом",resolvedAddress:f.address,facts:[{id:"f1",claim:"Старый факт"}]},draft:{title:"Непроверенный черновик",paragraphs:[]}};
  f.store.update(initial.id,{stage:"writing",data:initial.data},initial.revision);initial.revision++;
  f.queue.shift();

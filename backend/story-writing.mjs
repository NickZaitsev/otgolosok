...
export async function writeStory(evidence,{profile="story-v1",provider,address=evidence.resolvedAddress,signal,onCandidate=()=>{},onReview=()=>{}}={}) {
  ...
  try{links=acceptReview(review.value,draft.paragraphs,evidence);}
    ...
    review=await requestStructured(provider,reviewPrompt(address,{...draft,paragraphs:draft.paragraphs.map((p,index)=>({paragraph:index+1,text:p.text}))},evidence),{signal,timeoutMs:120000,maxTokens:1800});onReview(review.value);links=acceptReview(review.value,draft.paragraphs,evidence);
  }

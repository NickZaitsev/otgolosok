...
export function Account() {
  ...
  async function save(event: FormEvent) {
    ...
    try { const data = await accountApi("/api/me", { method: "PATCH", body: JSON.stringify({ name: name.trim() }) }); setUser(data.user); setName(data.user.name); setNotice("Имя сохранено."); }
  ...
  return <main className="ui-page profile-page"><AppHeader />
    ...
    {user && <>
      <header className="profile-hero"><div className="profile-avatar" aria-hidden="true">{user.name.trim().split(/\s+/).slice(0, 2).map(part => part[0]).join("").toLocaleUpperCase("ru")}</div><div><p className="ui-eyebrow">Личное пространство</p><h1>{user.name}</h1><p>{user.email}</p></div></header>
      ...
      <div className="profile-columns"><section className="profile-section"><h2>Профиль</h2><p className="ui-muted">Как к вам обращаться?</p><form onSubmit={save}><label className="ui-field">Ваше имя<input value={name} onChange={e => setName(e.target.value)} required maxLength={80} autoComplete="name" /></label><button className="ui-button" disabled={busy !== null || !name.trim() || name.trim() === user.name}>Сохранить изменения</button></form>{notice && <p className="ui-notice" role="status">{notice}</p>}</section>

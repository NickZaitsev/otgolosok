...
export function WalkBuilder() {
  ...
  return <main className="walk-builder">
    ...
    <section aria-labelledby="walk-route"><h2 id="walk-route">Куда пойдём</h2>
      ...
      {candidate ? <p className="walk-warning" role="status">Сначала подтвердите выбранный адрес <strong>{candidate.address}</strong> как начало или отмените выбор выше.</p> : null}
    ...
    <ResearchPanel draft={draft} current={current} persist={persist} offered={researchOffered && selection === "auto"} disabled={locked || !!candidate || !!storageError} chooseStartDisabled={locked} action={action} setBusy={setBusy} onApply={() => { setSelection("manual"); setReviewed(false); setResearchOffered(false); setError(""); }} onChooseStart={() => { edit({ start: null }); setTarget("start"); setCandidate(null); document.querySelector<HTMLInputElement>('input[placeholder="Москва, улица и номер дома"]')?.focus(); }} />

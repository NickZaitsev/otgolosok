...
export function WalkBuilder() {
  ...
  return <main className="walk-builder">
    ...
    <ResearchPanel draft={draft} current={current} persist={persist} offered={researchOffered && selection === "auto"} disabled={locked || !!candidate || !!storageError} action={action} setBusy={setBusy} onApply={() => { setSelection("manual"); setReviewed(false); setResearchOffered(false); setError(""); }} onChooseStart={() => { edit({ start: null }); setTarget("start"); setCandidate(null); document.querySelector<HTMLInputElement>('input[placeholder="Москва, улица и номер дома"]')?.focus(); }} />

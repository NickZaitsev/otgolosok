...
export function AppNavigation({ active, embedded = false, onNearby, onWalk }: Props) {
  ...
  const walk = <Link href="/?walk=create" onClick={onWalk} aria-current={current === "walk" ? "page" : undefined}><ExploreIcon name="walk"/><span>Прогулка</span></Link>;

  return <nav className={`app-navigation${embedded ? " around-nav" : " app-navigation--standalone"}`} aria-label="Основная навигация">
    {nearby}
    {walk}
    <Link href="/history" aria-current={current === "history" ? "page" : undefined}><ExploreIcon name="headphones"/><span>История</span></Link>
